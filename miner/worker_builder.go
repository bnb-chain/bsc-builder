...
// commitWorkV2
// TODO(roshan) consider whether to take bundle pool status as LOOP_WAIT condition
...
LOOP:
	for _, tx := range txs {
		...
		logs, err := w.commitTransaction(env, &txpool.Transaction{Tx: tx}, core.NewReceiptBloomGenerator())
		switch err {
...
func (w *worker) simulateBundle(
	...
	for i, tx := range bundle.Txs {
		...
		if pendingTxs != nil {
			...
			if accountTxs, ok := pendingTxs[sender]; ok {
